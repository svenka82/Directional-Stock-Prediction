...
def four_hour_window_model():
    ...
    print("Random Forest Accuracy: " + str((cm[0, 0] + cm[1, 1]) / (cm[0, 0] + cm[1, 1] + cm[1, 0] + cm[0, 1])))

    svc = SVC(kernel='rbf', random_state=0)
    svc.fit(X_train, y_train)
    y_pred = svc.predict(X_test)
    cm = confusion_matrix(y_test, y_pred)
    print("SVM Accuracy: " + str((cm[0, 0] + cm[1, 1]) / (cm[0, 0] + cm[1, 1] + cm[1, 0] + cm[0, 1])))

    mlp = MLPClassifier(hidden_layer_sizes=(300, 300, 300, 300), random_state=10)
    mlp.fit(X_train, y_train)
    y_pred = mlp.predict(X_test)
    cm = confusion_matrix(y_test, y_pred)
    print("MLP Accuracy: " + str((cm[0, 0] + cm[1, 1]) / (cm[0, 0] + cm[1, 1] + cm[1, 0] + cm[0, 1])))
